...
$('document').ready(function() {
	...
	$('#searchButton').on('click', requestBookList);
	$('.w3-paging-button').on('click', function() {
		var page = $(this).attr('page');
		requestBookList(page);
	});
});

//책 리스트를 요청한다.
function requestBookList(page) {
	
	if (page == '') {
		page = 1;
	}
	
	var keyword = $('#searchBar-bottom').val();
	var searchOption = $('#searchOption').val();
	if ($('#searchBar-bottom-hidden').val() != '') {
		keyword = $('#searchBar-bottom-hidden').val();
		searchOption = $('#searchOption-hidden').val();
	}
	...
	if (keyword != '') {
		...
		href += 'searchOption='+searchOption+'&';
	}
...
<div class="w3-content" style="max-width:2000px;margin-top:46px">
	...
	<h2 class="w3-wide w3-hover-opacity" id="book"><a href="list">소설</a></h2>
	
	<!-- 책 목록 구역 -->
	<div class="w3-row w3-padding-32" id="Div-bookList">
		<c:if test="${resultMap.bookList.size() > 0 }">
 			<c:forEach items="${resultMap.bookList }" var="book">
 				<c:if test="${book.isbn.length() > 13 }">
					<c:set var="isbn" value="${book.isbn.split(' ')[1] }" />
				</c:if>
				<c:if test="${book.isbn.length() <= 13 }">
					<c:set var="isbn" value="${book.isbn }" />
				</c:if>
				<div class="w3-third" >
					<p id="book${book.isbn }">${book.title }</p>
					<c:if test="${book.image != ''}">
					<!-- 책 이미지가 있는 경우 -->
						<img alt="" src="${book.image }" class="w3-round w3-margin-bottom w3-hover-opacity w3-books" 
						alt="Book" style="width:60%" onclick="location.href='info?isbn=${isbn }'">
					</c:if>
					<c:if test="${book.image == ''}">
					<!-- 책 이미지가 없는 경우 -->
						<img alt="" src="../resources/images/book_default.png" class="w3-round w3-margin-bottom w3-hover-opacity w3-books" 
						alt=	"Book" style="width:60%" onclick="location.href='info?isbn=${isbn }'">
					</c:if>
				</div>
			</c:forEach>
    ...
    <!-- 페이지 처리 구역 -->
	<div id="Div-paging">
		<c:if test="${resultMap.page > 10 }">
			<span class="w3-medium w3-paging-button" page="1">처음</span>
			<span class="w3-medium w3-paging-button" page="${resultMap.startPage - 1 }">이전</span>
		</c:if>
		<c:forEach var="i" begin="${resultMap.startPage }" end="${resultMap.endPage }">
			<c:if test="${resultMap.page == i}">
				<span class="w3-medium w3-paging-button" page="${i }"><b>${i }</b></span>
			</c:if>
			<c:if test="${resultMap.page != i}">
				<span class="w3-medium w3-paging-button" page="${i }">${i }</span>
			</c:if>
		</c:forEach>
 		<c:if test="${resultMap.currentGroup+1 < (resultMap.totalPage/10) }">
			<span class="w3-medium w3-paging-button" page="${resultMap.endPage + 1 }">다음</span>
			<span class="w3-medium w3-paging-button" page="${resultMap.totalPage }">끝</span>
		</c:if>
	</div>
	...
	<div class="Div-search">
		...
		<input type="search" id="searchBar-bottom">
		<input type="hidden" id="searchBar-bottom-hidden" value="${resultMap.keyword }">
		<input type="hidden" id="searchOption-hidden" value="${resultMap.searchOption }">
